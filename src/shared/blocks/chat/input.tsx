...
import {
  ...
  type PlanModelView,
} from '@/shared/services/model-supply-ui';
...
export function ChatInput({
  ...
  const staticModels: ExtendedChatModel[] =
    STATIC_CHAT_MODELS as ExtendedChatModel[];
  ...
  const [userPlan, setUserPlan] = useState<PlanModelView>({
    plan: 'free',
  ...
  const uiModels = useMemo<ModelOption[]>(() => {
    return buildAllowedModelMenu({
      staticModels,
      difyBots,
      plan: userPlan,
    }) as ModelOption[];
  }, [difyBots, staticModels, userPlan]);

  useEffect(() => {
    if (uiModels.length === 0) return;
    if (!model || !uiModels.some((item) => item.id === model)) {
      setModel(uiModels[0].id);
    }
  }, [model, uiModels]);
  ...
  const handleSendMessage = useCallback(
    async (data: {
      ...
      if (!currentUiModel) {
        toast.error('This model is not available on your current plan.', {
          action: {
            label: 'Upgrade',
      ...
      try {
        const provider =
          model === 'auto' ? 'auto' : selectedModel?.provider || 'openrouter';
