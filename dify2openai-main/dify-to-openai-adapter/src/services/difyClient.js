          ...
          if (line.trim().startsWith('data: ')) {
            try {
              ...
              const data = JSON.parse(jsonStr);
                ...
                case 'node_started': {
                  const nodeTitle = data.data?.title || data.data?.node_id || 'unknown-node';
                  const nodeType = data.data?.node_type || 'unknown-type';
                  const nodeId = data.data?.node_id || null;
                  console.log(`[${this.appName}] Node started: ${nodeTitle} (${nodeType})`);
                  if (showNodeEvents) {
                    writeSseEvent('node', {
                      status: 'started',
                      title: nodeTitle,
                ...
                case 'node_finished': {
                  ...
                  const nodeType = data.data?.node_type || 'unknown-type';
                  const nodeId = data.data?.node_id || null;
                  const elapsedMs = data.data?.elapsed_time ?? data.data?.elapsed_ms ?? data.data?.duration ?? null;
                  console.log(`[${this.appName}] Node finished: ${nodeTitle} (${nodeType})`);
                  let outputText = '';
                  if (data.data?.outputs && typeof data.data.outputs === 'object') {
                    outputText = this.extractTextFromOutputs(data.data.outputs) || '';
                    if (outputText) {
                      console.log(`[${this.appName}] Node output (captured): "${outputText.substring(0, 100)}..."`);
                    }
                  }
                  if (showNodeEvents) {
                    const trimmedOutput = outputText.length > 2000
